...
class EleveController extends Controller {
    ...
    public function ajax() {
        ...
        $inscriptions = $this->Inscription->getAllInscriptions($this->request->ideleve);
        # Rechercher les notes de cette eleve pour chaque inscription
        $notes = new ArrayObject();
        $notations = array();
        foreach ($inscriptions as $inscr) {
            $note = $this->Note->getNotesEleveByClasse($this->request->ideleve, $inscr['IDCLASSE']);
            foreach ($note as $n) {
                $notations[] = $this->Notation->getBy(["IDNOTATION" => $n['NOTATION']]);
            }
            $notes->offsetSet($inscr['IDCLASSE'], $note);
        }
        $view->Assign("notations", $notations);
        $view->Assign("classes", $inscriptions);
        $view->Assign("tableNotes", $notes->getArrayCopy());
